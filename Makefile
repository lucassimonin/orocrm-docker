...
config/parameters.yml: config/parameters.yml.dev
	@if [ -f config/parameters.yml ]; \
	then\
		echo '\033[1;41m/!\ The parameters.yml.dev file has changed. Please check your parameters.yml file (this message will not be displayed again).\033[0m';\
		touch config/parameters.yml;\
		exit 1;\
	else\
		echo cp config/parameters.yml.dev config/parameters.yml;\
		cp config/parameters.yml.dev config/parameters.yml;\
	fi


.PHONY: build kill install reset start start-certbot stop clean no-docker deploy
##
## Utils
## -----
##
...
.PHONY: db assets watch clear flush console sync-ssl
